...
export function monthDrawInstant(date = new Date()) {
  const parts = londonParts(date);
  return londonToUtc(Number(parts.year), Number(parts.month), 1, 20, 0);
}

export function isDrawDue(now = new Date()) {
  return now.getTime() >= monthDrawInstant(now).getTime();
}
